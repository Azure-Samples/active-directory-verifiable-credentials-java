    ...
    this.pollRequestStatus = function (id) {
        ...
        var pollFlag = setInterval(async function () {
            ...
            if ((tmNow - 10) > _rsThis.request.expiry) {
                ...
                _rsThis.onError( _rhThis.requestType, { error: "timeout", error_description: `The ${_rsThis.requestType} request was not process in time.` });
                return;
            }
            ...
            if (respMsg.error_description) {
                clearInterval(pollFlag);
                _rsThis.onError(_rhThis.requestType, respMsg.error_description);
            } else {
