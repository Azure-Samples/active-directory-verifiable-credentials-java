...
#> 
    ...
    Write-Host "Creating the AAD application (Verifiable Credentials Java sample)"
    $clientAadApplication = New-AzADApplication -DisplayName "Verifiable Credentials Java sample" `
                                                -IdentifierUris "https://$tenantDomainName/vcjavasample" 
    ...
    Write-Host "Done creating the client application (VC Java sample)"
    ...
    Add-Content -Value "<tr><td>client</td><td>$($clientAadApplication.AppId)</td><td><a href='$clientPortalUrl'>VC Java sample</a></td></tr>" -Path createdApps.html
    ...
    $files = @( "run.cmd", "run.sh", "docker-run.cmd", "docker-run.sh")
    foreach( $file in $files ) {
      $configFile = $pwd.Path + "\..\" + $file    
